...
# Re-cut kit-owned M3 engagement holes in the new corner blocks; top/base through holes stay on the same axes.
...
for sy in (-1,1):
    ...
    for sx in (-1,1):
        ...
        bj=_slim_make_screw_joint_v1(size='M3',at=Location((x,y,-foot_height),(180,0,0)),through=[(base,base_t+foot_height)],engage_depth=slim2_corner_engagement,into=rebuilt,head='socket_cap',strategy='auto',material=material,boss='none',label=f'slim2-bottom:{sy}:{sx}')
        base=(base-bj.through_cuts[0]).clean(); rebuilt=(rebuilt-bj.engage_cuts).clean()
        tj=_slim_make_screw_joint_v1(size='M3',at=Location((x,y,H)),through=[(top_cap,cap_t+cap_inner_t)],engage_depth=slim2_corner_engagement,into=rebuilt,head='socket_cap',strategy='auto',material=material,boss='none',label=f'slim2-top:{sy}:{sx}')
...
publish('base',base,'Slim four-point bottom'); publish('top_cap',top_cap,'Slim four-point top'); publish('front_panel',front_panel,'Slim 10 mm magnetic front'); publish('rear_panel',rear_panel,'Slim 10 mm magnetic rear')
print(f'SLIM_INTERFACE_REBUILD_PASS: screwless magnet carriers are {magnet10_rebuild_carrier_depth:.1f}x{magnet10_rebuild_carrier_width_y:.1f}x{magnet10_rebuild_carrier_height_z:.1f} mm; corner blocks are {slim2_corner_width_x:.0f}x{slim2_corner_depth_y:.0f}x{slim2_corner_height_z:.0f} mm.')